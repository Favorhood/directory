...
app.controller('ScrollCtrl', function ($scope, $http, $ionicScrollDelegate, $interval, flickr, $ionicPopup, $state, filterFilter) {
    ...
    $scope.swipeRight = function () {
        ...
        if (scrollposition == 0) {
            var index = ($scope.totalElement - 1) - $scope.reverseElement % $scope.totalElement;
...
app.controller('FlickrCtrl', function ($scope, $http, $ionicScrollDelegate, $interval, flickr, $ionicPopup, $state, filterFilter) {
    
    $scope.isde = $ionicScrollDelegate.$getByHandle('isdeflickr');
    $scope.reverseElement = 0;
    $scope.toscroll = undefined;
    $scope.feed = flickr;
    $scope.items = [];
